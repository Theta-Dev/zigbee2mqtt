...
class Controller {
    ...
    async publishEntityState(IDorName, payload, stateChangeReason=null) {
        ...
        const options = {
            retain: utils.getObjectProperty(resolvedEntity.settings, 'retain', settings.get().mqtt.retain),
            qos: utils.getObjectProperty(resolvedEntity.settings, 'qos', settings.get().mqtt.qos),
        };
